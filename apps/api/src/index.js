...
app.post("/api/search", async (c) => {
  ...
  if (!query) return c.json({ query: "", channels: [] });

  try {
    // We use a flat search to get channels quickly
    const { stdout } = await execAsync(
      `yt-dlp "ytsearch10:${query}" --flat-playlist --dump-single-json --quiet --no-warnings`
    );
    ...
    // Sort by sub count (descending)
    channels.sort((a, b) => b.subCount - a.subCount);

    return c.json({ query, channels });
  } catch (error) {
